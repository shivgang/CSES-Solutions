...
int findCount(int n,int index,int sum){
    if(index == n){
        return sum == n;
    }

    if(dp[index][sum]!=-1)  return dp[index][sum];
    int notIncluded = findCount(n,index+1,sum) ;
    int included = 0;
    if(index <= sum)
        included = findCount(n,index+1,sum-index);

    return dp[index][sum] = (notIncluded + included)%MOD ; 
}
