...
string add(const string &a, const string &b)
{
    ...
    for (; i >= 0 || j >= 0;)
    {
        ...
        carry = 0;
        if (i >= 0)
            number += charToDigit(a[i--]);
        if (j >= 0)
            number += charToDigit(b[j--]);
        carry = number / 10;
        number %= 10;
        s += digitToChar(number);
    }
    if (carry >= 1)
        s += to_string(carry);
...
void multiply(vector<int> &fact, int number, int &size)
{
    int carry = 0;
    for (int i = 0; i < size; i++)
    {
        int prod = fact[i] * number + carry;
        fact[i] = prod % 10;
        carry = prod / 10;
    }
    while (carry)
...
{
    vector<int> fact(1000, 0);
    fact[0] = 1;
    int size = 1;
    for (int i = 2; i <= n; i++)
        multiply(fact, i, size);
...
void computelps(string s, int *lps)
{
    ...
    {
        if (s[i] == s[len])
        {          // if characters equal we can simply add this chaaracter
        ...
        {
            if (len == 0)
                lps[i++] = 0; // if len becomes 0 no lps exists
            else
                len = lps[len - 1]; // reduce len so as to find lps in smaller subarray
        }
    }
}
...
int rabinKarp(string source, string target)
{
    ...
    int hsource = 0, htarget = 0;
    for (int i = 0; i < m; i++)
    {
        hsource = (hsource * d + source[i]) % q;
        htarget = (htarget * d + target[i]) % q;
    }
    if (hsource == htarget && source.substr(0, m) == target)
        return 0;
    for (int i = m; i < n; i++)
    {
        hsource = (hsource - preCompute * source[i - m]) % q;
        hsource = (hsource * d + source[i]) % q;
        if (hsource < 0)
            hsource += q;
        if (hsource == htarget && source.substr(i - m + 1, m) == target)
            return i - m + 1;
    }
    return -1;
}
...
public:
    ...
    void update (int qlow , int qhigh , int value , int l , int r , int node){
        if (lazy [node] != 0){
            sumNodes [node] += (r - l + 1) * lazy [node];
            if (l != r){
        ...
        update ( qlow , qhigh , value , l , mid , 2 * node );
        update ( qlow , qhigh , value , mid + 1 , r , 2 * node + 1 );
    ...
    int query (int qlow , int qhigh , int l , int r , int node){
        ...
        if ( qlow <= l && r <= qhigh )    return sumNodes [node];
    ...
    void update (int qlow , int qhigh , int val){
        update (qlow , qhigh , val , 0 , n - 1 , 1);
    }

    int query (int qlow , int qhigh){
        return query(qlow , qhigh , 0 , n - 1 , 1);
    }
...
void solve()
{
    ...
    while (q--){
        ...
        if (type == 1){
            ...
            segment.update (a , b , val);
        }else{
            int k;
            cin >> k;
            cout << segment.query (k , k) << endl;
        }
    }
