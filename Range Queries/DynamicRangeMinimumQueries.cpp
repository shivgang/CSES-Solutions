...
public:
    ...
    void build(int node,int l,int r){
        ...
        minNode[node] = min (minNode[2*node] , minNode[2*node+1]);
    }
    ...
    int query(int queryL,int queryR,int l,int r,int node){
        if(queryL>r || queryR<l)    return INT_MAX;
        if(queryL<=l && queryR>=r)  return minNode[node];

        int mid = l + (r-l) /2;
        int leftMin = query(queryL,queryR,l,mid,2*node);
        int rightMin = query(queryL,queryR,mid+1,r,2*node+1);
...
void solve() {
    ...
    while (q--){
        ...
        if (type == 1){
            ...
            cin >> index >> value;
            index--;
            segment.update (index , value , 0 , n - 1 , 1);
        }else{
            int a , b;
            cin >> a >> b;
            a-- , b--;
            cout << segment.query(a , b , 0 , n - 1 , 1) << endl;
        }
    }
}
