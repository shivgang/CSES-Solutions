...
int preOrder(TreeNode* node,vector <int> &subordinates){
    if ( !node ){
        return 0 ;
    }
    ...
    for(auto child:node->children){
        count += preOrder(child , subordinates);   
    }
...
void solve() {
    ...
    for (int i = 2 ; i <= n ; i++){
        int node = i ;
        cin >> parent[i];
        nodes[parent[i]] -> children . push_back (nodes[node]);   
    }

    
    vector <int> subordinates( n + 1 );
    preOrder (nodes[1],subordinates);
