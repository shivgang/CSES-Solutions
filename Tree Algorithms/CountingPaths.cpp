...
vector <int> depth;
void makeTree(vector <int> &parent , int node , int par){
    for( auto neighbour : graph[node] ){
        if (neighbour != par){
            parent [neighbour] = node;
            makeTree(parent , neighbour , node);
        }
    }
}
 
void makeUPforLCA(const vector <int> &parent , int n){
    int MAX = 32;
    up . resize( 32 , vector <int> ( n + 1 ));
    depth . resize( n + 1  , 0);
...
void addPaths(const vector <int> &parent, vector<int> &paths, int node , int par){
    
    for(auto neighbour : graph [node]){
        if ( neighbour != par ){
            addPaths(parent , paths , neighbour , node);
            paths [node] += paths [neighbour];
        }
    }
